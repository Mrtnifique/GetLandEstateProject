...
public class US_05_SelinUserControllerStepDefs {
    ...
    public void authUserIcinGuncellemePayloadDuzenlenir() {
        ...
        patch.setFirstName("ali");
        patch.setLastName("can");
        patch.setEmail("alitestest@gmail.com");
        patch.setRole("ADMIN");
        patch.setPhone("(457) 149-5317");
        patch.setPassword(">%+S$=baCaF`08tc3)WZn<Ok*HIxqI");
    ...
    public void authUserIcinPATCHRequestGonderilir() {
        response = given(spec).body(patch).when().patch("{first}/{second}/{third}");
        response.prettyPrint();
    }
